...
def fetch_panorama_tile(
    tile_info: TileInfo, max_retries: int = DEFAULT_MAX_RETRIES, http_proxy: str | None = None
) -> Image.Image:
    """
    ...
    for _ in range(max_retries):
        try:
            response = requests.get(tile_info.fileurl, stream=True, headers=headers, proxies={"http": http_proxy, "https": http_proxy} if http_proxy else None)
...
async def fetch_panorama_tile_async(
    ...
    for _ in range(max_retries):
        try:
            response = await async_client.get(tile_info.fileurl, proxies={"http": http_proxy, "https": http_proxy} if http_proxy else None)
            response.raise_for_status()
            return Image.open(BytesIO(response.content))
...
def iter_tiles(
    ...
    if not multi_threaded:
        for info in iter_tile_info(pano_id, zoom):
            image = fetch_panorama_tile(info, max_retries, http_proxy=http_proxy)
            yield Tile(x=info.x, y=info.y, image=image)
        return

    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_tile = {
            executor.submit(fetch_panorama_tile, info, max_retries, http_proxy=http_proxy): info
            for info in iter_tile_info(pano_id, zoom)
        }
...
async def iter_tiles_async(
    pano_id: str, zoom: int, max_retries: int = DEFAULT_MAX_RETRIES, http_proxy: str | None = None
) -> AsyncGenerator[Tile, None]:
    for info in iter_tile_info(pano_id, zoom):
        image = await fetch_panorama_tile_async(info, max_retries, http_proxy=http_proxy)
        yield Tile(x=info.x, y=info.y, image=image)
    return
